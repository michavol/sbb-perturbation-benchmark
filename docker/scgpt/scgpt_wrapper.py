...
class SCGPTWrapper:
    ...
    def _scgpt_gene_info_path(self) -> Optional[Path]:
        """Location of scGPT reference gene list (for OOV masking), if present."""
        candidates = (
            Path("/app/data/ref/gene_info_scgpt.csv"),
            Path(__file__).resolve().parent / "data" / "ref" / "gene_info_scgpt.csv",
            Path(__file__).resolve().parent.parent.parent / "data" / "ref" / "gene_info_scgpt.csv",
        )
        for p in candidates:
            if p.exists():
                return p
    ...
    def _ensure_symbol_scgpt(self, adata: AnnData) -> None:
        """Ensure ``var['symbol_scgpt']`` exists in memory (does not modify the source h5ad on disk).

        Official CellSimBench builds include this column; some datasets omit it. We fill from
        ``gene_name`` (or ``var_names``), then mask symbols not in the scGPT reference list when
        ``gene_info_scgpt.csv`` is available (same pattern as wessels23 ``get_data.py``).
        """
        ...
        if gene_info_path is not None:
            ...
            log.info(
                "Masked %d genes not in scGPT reference vocabulary (gene_info: %s)",
                n_oov,
                gene_info_path,
            )
