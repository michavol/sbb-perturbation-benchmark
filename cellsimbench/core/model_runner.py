    ...
    def run_model(
        ...
        if model_type == 'docker':
            ...
            raise ValueError("Baselines are handled by BaselineRunner, not ModelRunner")
             ...
             return self._run_builtin_prediction(
                 model_config, data_manager, split_name, output_dir, gpu_id
             )
        else:
            raise ValueError(f"Unknown model type: {model_type}. Currently only 'docker' and 'builtin' are supported.")

    def _run_builtin_prediction(self, model_config: Dict[str, Any],
                                data_manager: DataManager,
                                split_name: str,
                                output_dir: Path,
                                gpu_id: Optional[int] = None) -> Path:
        """Execute built-in model prediction locally."""
        from ..models.builtin_models import get_builtin_model
        
        model_name = model_config.get('builtin_model_name', model_config.get('name'))
        # If model name isn't directly the class key, maybe we map it?
        # Assuming 'name' in config refers to the specific instance (e.g. lse_univ_ols)
        # We need a field 'model_class' or 'builtin_type'?
        # Let's use 'builtin_name' in config if available, else try 'name'
        
        builtin_type = model_config.get('builtin_type', 'lse_regression') # Defaulting for now
        
        log.info(f"Running built-in model: {model_name} (type: {builtin_type})")
        
        ModelClass = get_builtin_model(builtin_type)
        model = ModelClass(model_config)
        
        # Test conditions from data manager
        # Note: ModelRunner's run_model receives model_config, but doesn't explicitly pass test_conditions
        # _run_docker_prediction extracts them. We should do the same.
        ...
        log.info(f"Built-in model predictions saved to {output_path}")
        return output_path
