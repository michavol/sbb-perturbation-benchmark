...
def benchmark_main(cfg: DictConfig) -> float:
    ...
    try:
        ...
        # Ensure training config exists for benchmarking
        if not hasattr(cfg, 'training'):
            from omegaconf import open_dict
            with open_dict(cfg):
                model_name = cfg.model.name if hasattr(cfg, 'model') else cfg.dataset.name
                cfg.training = {
                    'output_dir': f'models/{model_name}_{cfg.dataset.name}/',
                    'save_intermediate': True
                }
